...
export function AIChat() {
  ...
  const renderSessionItem = (session: ChatSession) => (
    <div
      ...
      className="flex items-center justify-between p-4 bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 cursor-pointer hover:border-blue-500 hover:shadow-md transition-all group"
    >
      <div className="flex-1 min-w-0">
          <div className="font-medium text-slate-800 dark:text-slate-200 truncate">{session.title || '无标题会话'}</div>
          <div className="text-xs text-slate-500 mt-1">更新于 {new Date(session.updatedAt).toLocaleString()}</div>
      </div>
      <button
        onClick={(e) => deleteSession(e, session.id)}
        className="opacity-0 group-hover:opacity-100 p-2 text-slate-400 hover:text-red-500 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full transition-all"
        title="删除对话"
      >
        <Trash2 size={16} />
      </button>
    ...
    return (
      <div className="mb-6">
        <button
          ...
          className="flex items-center w-full p-2 text-lg font-bold text-slate-700 hover:text-slate-900 dark:text-slate-300 dark:hover:text-slate-100 transition-colors mb-2"
        >
          {isExpanded ? <ChevronDown size={20} /> : <ChevronRight size={20} />}
          <span className="ml-2 flex items-center gap-2">
            {icon}
            {label}
            <span className="bg-slate-200 dark:bg-slate-700 text-sm px-2 py-0.5 rounded-full font-normal text-slate-600 dark:text-slate-400">{list.length}</span>
          </span>
        </button>
        {isExpanded && (
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-900 overflow-y-auto">
       <div className="p-8 max-w-7xl mx-auto w-full">
           <div className="mb-8">
                <h1 className="text-3xl font-bold text-slate-900 dark:text-slate-100 mb-2">AI 历史记录</h1>
                <p className="text-slate-600 dark:text-slate-400">查看所有的 AI 对话记录，点击即可跳转至对应场景继续对话。</p>
           </div>
           ...
           <div className="space-y-2">
                ...
                {data?.sessions.length === 0 && (
                    <div className="text-center text-slate-400 py-20 bg-slate-100 dark:bg-slate-800/50 rounded-xl border border-dashed border-slate-300 dark:border-slate-700">
                        暂无对话记录
                    </div>
                )}
           </div>
